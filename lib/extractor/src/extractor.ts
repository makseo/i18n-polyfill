...
export class ServiceParser extends AbstractAstParser {
   ...
   */
  protected _findTranslateServicePropertyName(
    constructorNode: ts.ConstructorDeclaration | ts.FunctionDeclaration,
  ): string {
    if (!constructorNode) {
      return null;
    }
  ...
  protected _findCallNodes(node: ts.Node, propertyIdentifier: string): ts.CallExpression[] {
    ...
    callNodes = callNodes.filter((callNode) => {
      ...
      const expression = callNode.expression as any;
